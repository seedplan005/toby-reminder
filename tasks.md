...
### Backend
- [x] `GET /api/reminders/counts` API (today, scheduled, all, flagged, completed)
- [x] 스마트 리스트 조건별 쿼리 최적화

### Frontend
- [x] 홈 화면 구현 (스마트 리스트 카드 그리드, 2열)
  - [x] 카드 컴포넌트 (원형 아이콘 + 이름 + 건수)
  - [x] 색상: 오늘=파랑, 예정=빨강, 전체=검정, 깃발=주황, 완료됨=회색
- [x] 사이드바 스마트 리스트 섹션 (아이콘 + 이름 + 건수 배지)
- [x] 스마트 리스트와 내 리스트 섹션 구분선
- [x] 스마트 리스트 클릭 시 필터된 리마인더 표시
  - [x] 오늘: 마감일이 오늘
  - [x] 예정: 마감일이 미래
  - [x] 전체: 모든 미완료
  - [x] 깃발: flagged=true
  - [x] 완료됨: completed=true
- [x] 사이드바 상단 검색 바 (제목/메모 기준 필터링)
- [x] Phase 4 통합 테스트
...
### Backend
- [x] `PATCH /api/reminders/reorder` - 리마인더 순서 변경 API
- [x] `PATCH /api/lists/reorder` - 리스트 순서 변경 API

### Frontend
- [x] 드래그 앤 드롭: 리마인더 순서 변경 (리스트 내)
- [x] 드래그 앤 드롭: 리스트 순서 변경 (사이드바)
- [x] 리스트 전환 트랜지션 (페이드 + 슬라이드, 150ms ease-out)
- [x] 사이드바 접기/펼치기 슬라이드 애니메이션
- [x] 리마인더 삭제: 스와이프 또는 컨텍스트 메뉴
- [x] Hover 효과 (행 배경색 변화)
- [x] 키보드 단축키 (N=추가, Delete=삭제, ↑↓=이동)
- [x] Phase 5 통합 테스트
